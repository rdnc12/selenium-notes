...
/*web elements: web elements are on a webpage. They form the entire webpage together Locators:
 To locate the web elements
  8 locators: id, name, classname, linktext, partiallinktext, tagName, xpath, cssLocators
   Basic Navigations:  get(); navigate().to(), back(), forward, refresh(), maximize();
   => driver.manage.window.maximize close(); quit();
   Thread.sleep(5000);=> put waits on the browser. In this example, wait 5 seconds.
   driver.findElement(By.locatorName(“locatorValue”));
   driver.sendKeys(“”); => sends data in an input box, type in the text box driver.click(); => to click on an elements*/
